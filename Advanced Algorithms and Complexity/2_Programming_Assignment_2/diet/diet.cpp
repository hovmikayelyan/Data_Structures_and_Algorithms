...
inline void SwapLines(Matrix &a, Column &b, std::vector<bool> &used_rows, Position &pivot_element)
{
...
void ProcessPivot(Matrix &a, Column &b, const Position &pivot_element)
{
  const int size = a.size();
...
inline void MarkPivotUsedAlready(const Position &pivot, std::vector<bool> &used_rows, std::vector<bool> &used_columns)
{
...
pair<int, Column> solve_equation(Equation equation)
{
  ...
  for (int steps = a.size(); steps; --steps)
  {
    ...
    ProcessPivot(a, b, pivot_element);
    MarkPivotUsedAlready(pivot_element, used_rows, used_columns);
  }

  int code = BackSubstitution(a, b);

  return {code, std::move(b)};
}

vector<vector<int>> GetSubsetss(const vector<int> &set, int m)
{
...
vector<Column> SolveAllEQ(int m, const Matrix &A, const vector<long double> &b)
{
  ...
  generate(nums.begin(), nums.end(), [&s]
           { return s++; });
  auto subsets = GetSubsetss(nums, m);
...
{
  prepare(n, m, A, b);
  vector<Column> solutions = SolveAllEQ(m, A, b);
